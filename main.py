...
from utils.dataset_handler import mnist
from utils.train_and_validation import TrainAndValidation

device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

dataloaders, classes_names = mnist.get_dataloaders(device)
...
client_model = architectures.LeNet('mnist', True, 2)
...
malicious_client = architectures.LeNet('mnist', True, 2)
...
server_model = architectures.LeNet('mnist', False, 2)
print('server_model object is successfully built, summary: \n')
summary(model=server_model, input_size=(6, 12, 12), batch_size=dataloaders['train'].batch_size)
